...
# 2. Database Koordinat Akurat UNIB
locations = {
...
with st.sidebar:
    ...
    # Perbaikan Kata & Logika Transportasi
    if "Kaki" in transport_mode:
        osrm_mode, speed_modifier, color, nama_kendaraan = "foot", 1.0, "#00ffff", "Jalan Kaki"
    elif "Sepeda" in transport_mode:
        osrm_mode, speed_modifier, color, nama_kendaraan = "foot", 0.4, "#ffff00", "Sepeda Kampus"
    elif "Motor" in transport_mode:
        osrm_mode, speed_modifier, color, nama_kendaraan = "foot", 0.25 + (traffic_level/1000), "#ff00ff", "Sepeda Motor"
    else:
        osrm_mode, speed_modifier, color, nama_kendaraan = "foot", 0.35 + (traffic_level/200), "#ff3300", "Mobil"
...
with col1:
    ...
    if origin == target:
        ...
        if dist_geodesic < 50:
            st.success("Target sudah terdeteksi sangat dekat dengan radius Anda saat ini.")
        elif traffic_level > 70 and "Mobil" in transport_mode:
            st.error("PERINGATAN: Jalur utama diprediksi macet total. Penggunaan mobil sangat tidak disarankan!")
        else:
            # Teks diperbaiki menjadi profesional
            st.info(f"Rute optimal dihitung menggunakan mode **{nama_kendaraan}**. Jalur ini diprioritaskan oleh sistem karena aman dan paling efisien.")
...
with col2:
    ...
    # Fitur baru: Otomatis Zoom In fokus ke Gedung agar tidak kelihatan laut
    m.fit_bounds([origin_coords, target_coords], padding=(30, 30))
    ...
    folium.PolyLine(
        ...
        opacity=0.9,
        dash_array='8, 8',
        tooltip=f"Jalur Transmisi ({nama_kendaraan})"
    ).add_to(m)

    # Fitur baru: use_container_width agar peta responsif dan tidak terpotong layar
    st_folium(m, use_container_width=True, height=550, returned_objects=[])

st.markdown("---")
st.caption("Aisyah Azzahrah_G1A024033 // DEPT: Informatics Engineering UNIB // V.2.3 (Final)")